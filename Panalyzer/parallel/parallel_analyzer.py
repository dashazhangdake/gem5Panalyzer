...
class parallel_ace:
    ...
    def multiprocessing_ace(self, threads, start, n_window, lineoffset):
        time_history = []
        ...
        while start < analyzing_length + 1:
            ...
            with concurrent.futures.ProcessPoolExecutor() as executor:
                results_this_thread = [executor.submit(ace_calculators(self.csvin, self.reg_count).
                                                       ...
                                                       sample_ratio, lineoffset) for i in range(threads)]
    ...
    def multiprocessing_ace_q(self, threads, start, n_window, lineoffset):
        time_history = []
        ...
        while start < analyzing_length + 1:
            ...
            with concurrent.futures.ProcessPoolExecutor() as executor:
                results_this_thread = [executor.submit(ace_calculators(self.csvin, self.reg_count).
                                                       ...
                                                       lineoffset) for i in range(threads)]
                for f in concurrent.futures.as_completed(results_this_thread):
                    if threads_group_counter < n_groups:
                        ace_temp = f.result()
                        ace_matrix_whole = np.hstack((ace_matrix_whole, ace_temp))
